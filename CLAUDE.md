...
| Checking that a service's UI actually renders, not just that its pod is Ready | The `homelab-ui` MCP server — see `## Claude Tooling in This Repo` below, and `docs/claude-tooling.md` for the full reference. `probe.py health` cannot see a broken UI behind a healthy pod. **Grafana is the exception** — it serves its own login behind Authelia, so drive it with `uv run pytest -m ui -k grafana` rather than by hand; typing the admin password into a browser tool call leaks it into the transcript. |
