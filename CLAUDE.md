...
- **The remote-ssh auto-approve depends on a package deployed outside this repo.**
  ...
  out of the hook rather than a stale local copy, so no command on that path is auto-approved —
  the shim `exec`s this process, so in practice that's a nonzero exit with the traceback on
  stderr, not a clean no-output exit; what Claude Code does with a hook that exits that way is
  outside this repo's control. `.claude/hooks/tests/test_claude_guard_import.py` is what proves
  the raise itself in CI.
