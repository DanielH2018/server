...
cd /home/ubuntu/server || {
  echo "block-footguns.sh: guard did not run (cd to /home/ubuntu/server failed) — asking instead of allowing" >&2
  echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"ask","permissionDecisionReason":"block-footguns.sh: guard did not run (cd to /home/ubuntu/server failed), so this call was not checked. Review it yourself."}}'
  exit 0
}
