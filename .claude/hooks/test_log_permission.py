...
def test_summarize_extracts_the_right_field_per_tool():
    ...
    assert m.summarize("Task", {"subagent_type": "Explore", "description": "find x"}) == "Explore: find x"
    assert m.summarize("Skill", {"skill": "vault-commit"}) == "vault-commit"


def test_summarize_handles_mcp_tools_via_first_keys():
    assert m.summarize("mcp__ctx__query", {"library_id": "react", "tokens": 5000}) == \
...
def test_classify_maps_pretooluse_to_call_out_of_scope_to_none():
    assert m.classify({"hook_event_name": "PreToolUse", "tool_name": "Bash",
                       "tool_input": {"command": "ls"}}) == \
        {"kind": "call", "tool": "Bash", "sum": "ls"}
    assert m.classify({"hook_event_name": "PreToolUse", "tool_name": "Read",
                       "tool_input": {"file_path": "/x"}}) is None


def test_classify_maps_permissionrequest_and_notification_to_asks():
    assert m.classify({"hook_event_name": "PermissionRequest", "tool_name": "Bash",
                       "tool_input": {"command": "npm i"}}) == \
        {"kind": "ask", "tool": "Bash", "sum": "npm i"}
    assert m.classify({"hook_event_name": "Notification", "notification_type": "permission_prompt",
                       "tool_name": "Bash"}) == \
        {"kind": "ask", "tool": "Bash", "sum": "(unattributed)"}
    assert m.classify({"hook_event_name": "Notification", "notification_type": "idle_prompt",
...
def test_summarize_collapses_windows_temp_paths():
    a = m.summarize("Bash", {"command": 'node "C:/Users/daniel/AppData/Local/Temp/qedit.js"'})
    b = m.summarize("Bash", {"command": 'node "C:/Users/daniel/AppData/Local/Temp/qedit2.js"'})
    assert a == 'node "<tmp>"'
    assert a == b
...
def test_summarize_collapses_tmp_and_backslash_temp_paths():
    ...
    assert m.summarize("Bash", {"command": 'node "C:\\Users\\daniel\\AppData\\Local\\Temp\\x.js"'}) == \
        'node "<tmp>"'


def test_summarize_collapses_macos_var_folders_temp_paths():
    assert m.summarize("Bash", {"command": "node /var/folders/xy/abc123def/T/qedit.js"}) == \
        "node <tmp>"
...
def test_apply_event_increments_calls_and_sets_first_last():
    ...
    m.apply_event(store, {"kind": "call", "tool": "Bash", "sum": "ls"}, "2026-06-17T10:00:00.000Z")
    m.apply_event(store, {"kind": "call", "tool": "Bash", "sum": "ls"}, "2026-06-17T10:01:00.000Z")
...
def test_apply_event_dedups_asks_within_window_and_reports_change():
    store = {"version": 1, "updated": None, "entries": {}}
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "npm i"}, "2026-06-17T10:00:00.000Z") is True
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "npm i"}, "2026-06-17T10:00:01.000Z") is False
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "npm i"}, "2026-06-17T10:00:06.000Z") is True
...
def test_apply_event_skips_unattributed_echo_of_recent_attributed():
    store = {"version": 1, "updated": None, "entries": {}}
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "npm i"}, "2026-06-17T10:00:00.000Z") is True
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "(unattributed)"}, "2026-06-17T10:00:00.500Z") is False
...
def test_apply_event_still_counts_standalone_unattributed():
    store = {"version": 1, "updated": None, "entries": {}}
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "(unattributed)"}, "2026-06-17T10:00:00.000Z") is True
    assert store["entries"]["Bash" + m.SEP + "(unattributed)"]["asks"] == 1


def test_apply_event_counts_distinct_attributed_prompts_seconds_apart():
    store = {"version": 1, "updated": None, "entries": {}}
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "cmd-a"}, "2026-06-17T10:00:00.000Z") is True
    assert m.apply_event(store, {"kind": "ask", "tool": "Bash", "sum": "cmd-b"}, "2026-06-17T10:00:01.000Z") is True
...
def test_prune_if_needed_is_noop_under_cap():
    store = {"version": 1, "updated": None, "entries": {
        "a": {"tool": "Bash", "sum": "a", "calls": 1, "asks": 0, "first": "x", "last": "2026-06-17T10:00:00.000Z"}
    }}
...
def test_prune_if_needed_drops_stale_entries_when_over_cap():
    ...
    for i in range(m.MAX_ENTRIES):
        store["entries"]["s" + str(i)] = {"tool": "Bash", "sum": "s" + str(i), "calls": 1, "asks": 0, "first": "x", "last": stale}
...
def _run_hook(store_path, payload):
    ...
    subprocess.run([sys.executable, _HOOK], input=json.dumps(payload),
                   text=True, env=env, check=True)


def test_e2e_call_then_prompt_yields_calls1_asks1(tmp_path):
    store_path = str(tmp_path / "permissions.json")
    _run_hook(store_path, {"hook_event_name": "PreToolUse", "tool_name": "Bash", "tool_input": {"command": "deploy.sh"}})
    _run_hook(store_path, {"hook_event_name": "PermissionRequest", "tool_name": "Bash", "tool_input": {"command": "deploy.sh"}})
...
def test_e2e_out_of_scope_and_malformed_input_do_not_write_store(tmp_path):
    store_path = str(tmp_path / "permissions.json")
    _run_hook(store_path, {"hook_event_name": "PreToolUse", "tool_name": "Read", "tool_input": {"file_path": "/x"}})
    # malformed stdin must not raise (exit 0); store stays absent
    env = dict(os.environ, PERMLOG_STORE=store_path)
    subprocess.run([sys.executable, _HOOK], input="not json", text=True, env=env, check=True)
...
def test_e2e_lock_contention_drops_event_without_touching_store(tmp_path):
    ...
    try:
        _run_hook(store_path, {"hook_event_name": "PreToolUse", "tool_name": "Bash", "tool_input": {"command": "x"}})
        assert not os.path.exists(store_path)
    finally:
