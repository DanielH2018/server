...
APPROVE_LOCAL = [
    ...
    (
        "dmesg -e",
        "dmesg -e is --reltime; only the capital E sets console logging (#2078)",
    ),
    ("dmesg -d", "dmesg -d is --show-delta; only the capital D sets console logging"),
    ("journalctl -u docker -n 50", "journalctl reads the journal"),
    ("journalctl --since=-1h", "journalctl with a valued read flag"),
    ("rg -n pattern /var/log/app.log", "rg with a path"),
...
REJECT_LOCAL = [
    ...
    ("dmesg -n 1", "dmesg -n sets the console log level (#2078)"),
    ("dmesg -n1", "dmesg -n1 — the level glued to the flag"),
    ("dmesg --console-level=1", "dmesg --console-level with a glued value"),
    ("dmesg -D", "dmesg -D disables console logging"),
    ("dmesg --console-off", "dmesg --console-off disables console logging"),
    ("dmesg -E", "dmesg -E enables console logging"),
    ("dmesg --console-on", "dmesg --console-on enables console logging"),
    ("dmesg -TnD", "dmesg -TnD — n and D hidden in a short cluster"),
    (
        "journalctl --vacuum-size=100M",
        "journalctl --vacuum-size deletes archived journals",
    ),
    ("journalctl --rotate", "journalctl --rotate rotates the journal"),
    ("rg --pre=/tmp/x pattern", "rg --pre= runs a preprocessor per file"),
    (
        "nvidia-smi",
