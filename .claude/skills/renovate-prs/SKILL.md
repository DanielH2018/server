...
**Read the title and the branch together**, with `gh pr list --author app/renovate --json
number,title,headRefName`. The parenthetical is the fastest tell, but Renovate keeps the group
name in the title only for a group holding more than one dependency: a single-dependency group is
titled `Update <dep> …` and its group name survives only in the branch slug (#2641). #2620 read
`Update klutchell/unbound Docker tag to v1.26.1` and its branch is what named it a denied role's
pin. A title reading `Update n8n (manual — append
