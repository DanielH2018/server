---
description: Create a pull request from current changes — reads the full diff, summarizes it, waits for review, then pushes and opens the PR.
---
...
1. **Gather context** — run these in parallel:
   - `git diff HEAD` to get the full diff of all changes (staged + unstaged)
...
2. **Summarize the changes** — read the full diff carefully and produce:
...
3. **Present for review** — show the user:
   ...
   - A summary of which files are changed
