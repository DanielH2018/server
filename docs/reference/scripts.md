...
209 first-party script(s) in `scripts/`. Each summary is the script's own module docstring — change the docstring to change this page.
...
**1 of the 49 scripts that run unattended have no test; 8 of all 209 do not.** The first number is the one that matters. An untested script a person runs fails in front of that person; an untested one a cron or a commit gate runs fails unattended, or blocks everybody.
...
|---|---|---|---|
| `scripts/docs/reference/backlog.py` | Generate docs/reference/backlog.md — the findings Claude filed as GitHub Issues. | build_docs.py (a cron runs it unattended) | `test_gen_reference_backlog.py` *(indirect)* |
...
| `scripts/deploy.sh` | the entry point every doc, skill, hook and consumer names; it execs deploy_run.py. | deploy: ansible/roles/k8s/monitor-bridge/files/gitops_markers.py | `test_deploy_at_sha.py` *(indirect)* |
| `scripts/deploy_tools/deploy_detach_notify.py` | Post-deploy notifier for `scripts/deploy.sh --detach`. | deploy_run.py (every commit, CI run, deploy or Claude session runs it) | `test_deploy_detach_notify.py` |
| `scripts/deploy_tools/deploy_run.py` | Run an interactive Ansible deploy under the locks the automated deployers take. | every deploy (deploy.sh) | `test_deploy_staleness_precedes_tag_validation.py` *(indirect)* |
| `scripts/deploy_tools/deploy_staleness.py` | Refuse a deploy from a git tree that is behind origin/master. | deploy_run.py (every commit, CI run, deploy or Claude session runs it) | `test_deploy_staleness.py` |
| `scripts/deploy_tools/deploy_tags.py` | Validate the --tags a deploy was given, before Ansible silently accepts them. | deploy: ansible/roles/setup/gitops_deploy/files/deploy_narrow.py | `test_deploy_tags.py` |
| `scripts/backup/etcd_restore_drill.sh` | prove an off-box etcd snapshot actually restores, without an outage. | deploy: ansible/roles/setup/hypervisor/tasks/etcd_drill.yml | `test_etcd_restore_drill_cron.py` *(indirect)* |
| `scripts/deploy_tools/fact_cache_guard.py` | Clear the shared Ansible fact cache when it pins another worktree's interpreter. | deploy_run.py (every commit, CI run, deploy or Claude session runs it) | `test_fact_cache_guard.py` |
| `scripts/dev/fact_status.py` | Status, verification and lint for the fact stores — the one entry point. | prek hook (every commit) | `test_facts_cli.py` *(indirect)* |
| `scripts/dev/gen_hook_settings.py` | Render the `hooks` key of `.claude/settings.json` from the hook files' own declarations. | prek hook (every commit) | `test_gen_hook_settings.py` |
...
| `scripts/deploy_tools/narrow_setup.py` | Which narrow `--tags` value a setup-role change needs, or a refusal to guess. | deploy: ansible/roles/setup/gitops_deploy/files/deploy_narrow.py | `test_narrow_setup.py` |
...
119 script(s) — imported by another script — not an entry point.

| Script | What it does | Reached by | Tests |
|---|---|---|---|
| `scripts/diagnostics/probe_lib/alerts.py` | `probe.py alerts` -- DOWN history reconstructed from Loki, since Kuma keeps only current state. | imported by probe.py, subcommands.py | `test_probe_alerts.py` *(indirect)* |
| `scripts/lib/ansible_jinja_compat.py` | Ansible's `bool` filter, reimplemented for a vanilla Jinja2 environment. | imported by k8s_context.py | `test_validate_shell_templates.py` *(indirect)* |
| `scripts/lib/ansible_jinja_env.py` | The Jinja environment every render guard uses, carrying ansible-core's own filters. | imported by compose_templates.py, config_templates.py, k8s_manifests.py, setup_templates.py, shell_lint.py, shell_templates.py, unit_templates.py | `test_ansible_jinja_env.py` |
| `scripts/diagnostics/probe_lib/arr.py` | `probe.py arr <app> <api-path>` — read-only *arr API GETs against sonarr/radarr/prowlarr. | imported by cli_parser.py, postflight.py, probe.py, subcommands.py | `test_probe_arr_redaction.py` *(indirect)* |
...
| `scripts/docs/catalog_model.py` | The service catalogue's row type, its "cannot be derived" marker, and its path anchors. | imported by catalog_backup.py, catalog_facts.py, catalog_render.py, gen_role_glance.py, service_catalog.py | `test_catalog_backup.py` *(indirect)* |
| `scripts/docs/catalog_render.py` | The two renderings of the service catalogue: the MkDocs page and the standalone HTML. | imported by service_catalog.py | `test_service_catalog.py` *(indirect)* |
...
| `scripts/dev/findings_lib/claim.py` | Whether a claim on an issue is still live, decided from the worktree that holds it. | imported by claim_cli.py, findings.py | `test_findings_claim_record.py` *(indirect)* |
| `scripts/dev/findings_lib/claim_cli.py` | The four claim subcommands: `claim`, `release`, `claims` and `reap`. | imported by findings.py | `test_findings.py` *(indirect)* |
...
| `scripts/dev/findings_lib/cli.py` | The argparse construction for `findings.py`: every subparser, no boundary calls. | imported by findings.py | `test_findings.py` *(indirect)* |
| `scripts/diagnostics/probe_lib/cli_parser.py` | probe.py's argparse surface: every subparser, plus the `cert` openssl stage builder. | imported by curl_pipeline.py, probe.py | `test_probe.py` *(indirect)* |
| `scripts/lib/cli_registry.py` | A small named-entry registry shared by this repo's CLI dispatchers. | imported by subcommands.py | `test_cli_registry.py` |
...
| `scripts/diagnostics/probe_lib/core.py` | Shared plumbing for probe's subcommands: endpoints, secrets, HTTP, durations. | imported by _alert_fixtures.py, alerts.py, arr.py, b2_api.py, b2_ledger.py, cert_expiry.py, cli_parser.py, curl_pipeline.py, fetch_grafana_dashboards.py, ha.py, ha_state_model.py, health.py, health_docker.py, longhorn.py, metrics.py, monitors.py, pi_plane.py, postflight.py, probe.py, readonly_rbac.py, ui_login.py | `test_probe.py` *(indirect)* |
| `scripts/lib/cron_checks.py` | The three cron-environment rules a rendered shell template must satisfy. | imported by shell_templates.py | `test_shell_template_cron_rules.py` *(indirect)* |
...
| `scripts/deploy_tools/deploy_detach.py` | `deploy.sh --detach`: take the locks here, then run the playbook in a forked child. | imported by deploy_run.py | `test_deploy_annotations.py` *(indirect)* |
| `scripts/deploy_tools/deploy_playbook.py` | The playbook run of a foreground deploy, and the annotation a finished one leaves. | imported by deploy_detach.py, deploy_under_locks.py | `test_deploy_annotations.py` *(indirect)* |
| `scripts/deploy_tools/deploy_under_locks.py` | The locked half of a foreground deploy: tree lock, snapshot, service locks, playbook. | imported by deploy_detach.py, deploy_run.py | `test_deploy_lock_wait_budget.py` *(indirect)* |
| `scripts/lib/deployer_park.py` | What the GitOps deployer's own markers say it has deferred: a park, or a pending role. | imported by deploy_staleness.py, land_tags.py | `test_deployer_park.py` |
...
| `scripts/dev/foreign_owned.py` | Find the paths under a directory that another uid owns, and say how to clear them. | imported by prune_worktrees.py | `test_foreign_owned.py` |
...
| `scripts/dev/findings_lib/gh_calls.py` | The gh reads and writes `findings.py` makes, and the one place a plan is executed. | imported by backlog.py, claim_cli.py, findings.py | `test_findings.py` *(indirect)* |
| `scripts/lib/git.py` | One way to run git from a script, with the repository chosen by ``cwd`` alone. | imported by await_ci.py, clean.py, decisions.py, deploy_run.py, deploy_staleness.py, deploy_tags.py, deploy_under_locks.py, doc_freshness.py, docs_provenance.py, fact_status.py, fanout_place.py, narrow_broad.py, narrow_git.py, narrow_setup.py, narrow_setup_index.py, prune_worktrees.py, publish_pr.py, pytest_shard.py, releases.py, releases_diff.py, render_guard.py, renovate_branch_sweep.py, rotation_tools.py, shared_role_reach.py, state.py, tools.py | `test_git.py` |
| `scripts/secrets_mgmt/git_dates.py` | When each secret's ciphertext last changed, read out of git. | imported by secret_rotation.py | `test_secret_git_dates.py` *(indirect)* |
| `scripts/lib/gitops_markers.py` | The deployer's state directory, its marker basenames, and the parsers for their formats. | imported by deployer_park.py, gitops_state.py, k3s_upgrade_gates.py, land_tags.py, options.py, state.py | `test_deployer_park.py` *(indirect)* |
| `scripts/docs/glance_facts.py` | The setup-plane and Pi-plane fact readers behind `gen_role_glance.py`, and what every plane shares. | imported by gen_role_glance.py | `test_gen_role_glance.py` *(indirect)* |
...
| `scripts/diagnostics/probe_lib/ha.py` | Home Assistant: live state, automations, and the read-only WebSocket trace client. | imported by ha_state_model.py, postflight.py, probe.py, subcommands.py | `test_probe_ha.py` *(indirect)* |
...
| `scripts/infra_map/inventory.py` | Declared state: what ``containers_list`` and the role trees say should run. | imported by gen_infra_map.py, model.py | `test_gen_infra_map.py` *(indirect)* |
| `scripts/lib/invocation_sites.py` | Where a `scripts/...` path can be executed from, read once for two different questions. | imported by script_classify.py | `test_invocation_sites.py` |
| `scripts/dev/findings_lib/issue_model.py` | The finding vocabulary and the pure reads over a gh issue: no gh, no shell, no argv. | imported by _findings_fakes.py, backlog.py, claim.py, claim_cli.py, cli.py, findings.py, gh_calls.py, plans.py, verify.py | `test_findings.py` *(indirect)* |
...
| `scripts/lib/facts/lock.py` | ``docs/facts.lock``: the recorded hashes for every verified CLAUDE.md section. | imported by fact_status.py | `test_facts_lock.py` *(indirect)* |
...
| `scripts/deploy_tools/narrow_setup_index.py` | One setup role's task files, templates and vars, read at a single git ref. | imported by narrow_setup.py | `test_narrow_setup_facts.py` *(indirect)* |
| `scripts/deploy_tools/narrow_setup_playbook.py` | What a playbook and a task list say about tags, without touching git. | imported by narrow_setup.py, narrow_setup_index.py | `test_narrow_setup.py` *(indirect)* |
...
| `scripts/diagnostics/probe_lib/pi_plane.py` | `probe.py targets --pi` and `probe.py pi containers` — first-command triage for daniel-pi. | imported by probe.py, subcommands.py | `test_probe_pi_plane.py` *(indirect)* |
...
| `scripts/dev/findings_lib/plans.py` | What `findings.py` decides to do, as gh argv nobody has run yet. | imported by claim_cli.py, findings.py, gh_calls.py | `test_findings_claim_plans.py` *(indirect)* |
| `scripts/diagnostics/probe_lib/readonly_rbac.py` | `probe.py readonly-rbac` — is the read-only ServiceAccount still read-only? | imported by probe.py, subcommands.py | `test_probe_readonly_rbac.py` *(indirect)* |
| `scripts/lib/facts/relations.py` | The status algebra: the spec's Datalog rules as set comprehensions over finite relations. | imported by fact_status.py | `test_facts_relations.py` *(indirect)* |
| `scripts/lib/release_bin_groups.py` | Resolve which source files a `release_bin.yml` group deploys. | imported by cron_targets.py | `test_release_bin_groups_have_no_secrets.py` *(indirect)* |
| `scripts/diagnostics/probe_lib/releases.py` | `probe.py releases` -- which commit produced the manifests each k8s service is running. | imported by health.py, probe.py, subcommands.py | `test_probe_releases.py` *(indirect)* |
| `scripts/diagnostics/probe_lib/releases_consumers.py` | Which services a byte-supplying shared k8s role can actually make stale. | imported by releases.py | `test_probe_releases_consumers.py` *(indirect)* |
| `scripts/diagnostics/probe_lib/releases_diff.py` | The one staleness question a path cannot answer: did this `tasks/` diff move any bytes? | imported by releases.py | — |
...
| `scripts/diagnostics/probe_lib/subcommands.py` | The `SUBCOMMANDS` table and the `REGISTRY` built from it, for `probe.py --list`. | imported by probe.py | `test_probe_registry.py` *(indirect)* |
| `scripts/deploy_tools/land_lib/tick.py` | Step 4, the GitOps tick, retried while the unit's own flock gives up. | imported by deploy.py, health_verdict.py, pipeline.py | `test_land_abandoned_tick_watch.py` *(indirect)* |
| `scripts/deploy_tools/land_lib/tools.py` | Every process boundary a landing crosses, as one injectable object. | imported by _land_fakes.py, land.py, landing.py | `test_land_tools.py` *(indirect)* |
...
| `scripts/dev/gen_gitops_markers.py` | Copy the deployer's `gitops_markers.py` into every tree that reads its markers. | no automated caller in the tree | `test_gitops_markers_copies.py` *(indirect)* |
| `scripts/docs/gen_role_glance.py` | Generate the mechanical half of every role's `## At a glance` block, in place. | no automated caller in the tree | `test_gen_role_glance.py` |
...
| `scripts/deploy_tools/k3s_etcd_restore_gates.py` | Run the stop conditions of `docs/k3s-etcd-restore.md` in order, exit code naming the first failure. | no automated caller in the tree | `test_k3s_etcd_restore_gates.py` |
| `scripts/deploy_tools/k3s_upgrade_gates.py` | Run the four stop conditions of `docs/k3s-upgrade.md` in order, exit code naming the first failure. | no automated caller in the tree | `test_k3s_upgrade_gates.py` |
| `scripts/dev/fanout_lib/launch.py` | Create the worktree, write the brief, start the transient service — spec §3. | no automated caller in the tree | `test_fanout_launch.py` *(indirect)* |
| `scripts/deploy_tools/longhorn_dr_gates.py` | Run the stop conditions of `docs/longhorn-disaster-recovery.md` in order, exit code naming the first failure. | no automated caller in the tree | `test_longhorn_dr_gates.py` |
| `scripts/deploy_tools/longhorn_upgrade_gates.py` | Run the stop conditions of `docs/longhorn-upgrade.md` in order, exit code naming the first failure. | no automated caller in the tree | `test_longhorn_upgrade_gates.py` |
| `scripts/dev/fanout_lib/manifest.py` | The run manifest: ~/.claude/fanout/<run-id>.json, outside every checkout — spec §4. | no automated caller in the tree | `test_fanout_clean.py` *(indirect)* |
...
| `scripts/deploy_tools/pinned_rotation_gates.py` | Run the stop conditions of the pinned-secret procedure in `docs/secret-rotation.md`, exit code naming the first failure. | no automated caller in the tree | `test_pinned_rotation_gates.py` |
| `scripts/dev/fanout_lib/placement.py` | Pure placement over (host, cap, current, live_agents) — spec 2026-09-06 §2. | no automated caller in the tree | `test_fanout_placement.py` *(indirect)* |
...
| `scripts/validate/refresh_vendored_schemas.py` | Re-download the vendored JSON schemas this repo checks rendered config against. | no automated caller in the tree | `test_validate_k8s_manifests.py` *(indirect)* |
| `scripts/dev/renovate_branch_sweep.py` | Census the `renovate/*` branches no open PR and no Dependency Dashboard entry speaks for. | no automated caller in the tree | `test_renovate_branch_sweep.py` |
| `scripts/validate/renovate_config.sh` | Validate renovate.json against a freshly-resolved Renovate, riding out npm's publish window. | no automated caller in the tree | `test_renovate_config_retry.py` *(indirect)* |
| `scripts/dev/renovate_rebase.py` | Tick the rebase checkbox in a Renovate PR's body so Renovate refreshes the branch. | no automated caller in the tree | `test_renovate_rebase.py` |
...
| `scripts/dev/fanout_lib/status.py` | Read every batch on one host in one call, and stop one — spec 2026-09-06 §4-5. | no automated caller in the tree | `test_fanout_status.py` *(indirect)* |
| `scripts/dev/fanout_lib/transport.py` | The transport seam: every process boundary the dispatcher crosses, as one injectable object. | no automated caller in the tree | `test_fanout_clean.py` *(indirect)* |
...
18 script(s) document how to invoke themselves. The rest take `--help`.
