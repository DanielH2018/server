...
generated_at: 2026-08-24 19:46 UTC
generated_sha: c043f5a3
---
...
|---|---|---|---|---|---|
| artifacts | k8s | artifacts.local.<domain> (+ public if k8s_public_route) | Authelia | no PVC (stateless) | eligible |
| authelia | k8s | auth.local.<domain> (+ public if k8s_public_route) | none (public/no-auth) | daily -> R2 | denylisted (platform — SSO/OIDC gate; a failed deploy locks out access to everything behind it, including the tools to fix it) |
...
| freshrss | k8s | freshrss.local.<domain> (+ public if k8s_public_route) | Authelia | weekly -> B2 (default target) | eligible |
| headlamp | k8s | headlamp.local.<domain> (+ public if k8s_public_route) | Authelia | no PVC (stateless) | eligible |
| healthchecks | k8s | healthchecks.local.<domain> (+ public if k8s_public_route) | Authelia | weekly -> B2 (default target) | denylisted (observability — cron dead-man's-switch monitor. ALSO Recreate + RWO seed-volume PVC (migrating-state shape) — two independent reasons. COUPLING NOTE for a future promotion: check UUIDs here are baked into ping URLs in unrelated crons fleet-wide; a revert past a check's creation leaves those crons pinging a dead UUID, silently dropped) |
| home-assistant | k8s | home-assistant.local.<domain> (+ public if k8s_public_route) | none (public/no-auth) | daily -> R2 | eligible |
| homelab-mcp | k8s | no route (infra role) | none (public/no-auth) | no PVC (stateless) | eligible |
| homepage | k8s | homepage.local.<domain> (+ public if k8s_public_route) | Authelia | no PVC (stateless) | eligible |
| ical-proxy | k8s | ical-proxy.local.<domain> (+ public if k8s_public_route) | none (public/no-auth) | no PVC (stateless) | eligible |
...
| loki-homelab | k8s | loki-homelab.local.<domain> (+ public if k8s_public_route) | none (public/no-auth) | daily -> B2 (default group) | denylisted (observability — log store other monitors read from; a broken deploy blinds them without paging. ALSO Recreate + PVC on its own log data) |
| longhorn-ui | k8s | longhorn.local.<domain> (+ public if k8s_public_route) | Authelia | no PVC (stateless) | denylisted (renders no Deployment — route-only (IngressRoute + middlewares) in front of the Longhorn UI, which has no auth of its own; a bad route change risks exposing the storage control plane) |
...
| peanut | k8s | peanut.local.<domain> (+ public if k8s_public_route) | Authelia | no PVC (stateless) | eligible |
| pi-peer-backup | k8s | no route (infra role) | unknown (use_authelia not declared on this entry) | weekly -> B2 (default target) | eligible |
| pihole | k8s | pihole.local.<domain> (+ public if k8s_public_route) | Authelia | unknown (PVC present, claim name not statically resolvable: {{ inst.claim }}); daily -> B2 (default group) | denylisted (platform — LAN DNS resolver; a failed deploy breaks name resolution fleet-wide, and host probes stay green through that kind of outage) |
| prowlarr | k8s | prowlarr.local.<domain> (+ public if k8s_public_route) | Authelia | weekly -> B2 (default target) | eligible |
| qbittorrent | k8s | qbittorrent.local.<domain> (+ public if k8s_public_route) | Authelia | weekly -> B2 (default target); daily -> B2 (default group) | denylisted (state coupled outside the volume — reverting qbittorrent-config to a snapshot rewinds in-flight torrent bookkeeping while the media-data volume it references does not move; the pre-apply snapshot and revert work fine and are not the blocker) |
| radarr | k8s | radarr.local.<domain> (+ public if k8s_public_route) | Authelia | weekly -> B2 (default target); daily -> B2 (default group) | eligible |
...
| scrutiny | k8s | scrutiny.local.<domain> (+ public if k8s_public_route) | Authelia | daily -> B2 (default group); weekly -> B2 (default target) | denylisted (stateful / manual-upgrade — rolling branch tag on a stateful monitor, deliberately manual. ALSO Recreate + RWO seed-volume PVC (migrating-state shape) — two independent reasons) |
| sonarr | k8s | sonarr.local.<domain> (+ public if k8s_public_route) | Authelia | weekly -> B2 (default target); daily -> B2 (default group) | eligible |
| speedtest | k8s | speedtest.local.<domain> (+ public if k8s_public_route) | Authelia | daily -> B2 (default group) | eligible |
| tdarr | k8s | tdarr.local.<domain> (+ public if k8s_public_route) | Authelia | weekly -> B2 (default target); daily -> B2 (default group) | denylisted (state coupled outside the volume — rewrites media-data (shared RWX, not reverted) in place, an irreversible transcode the snapshot/revert can't undo; ALSO non-atomic two-claim revert (tdarr-configs, tdarr-server); ALSO the digest pin's 'stays manual' intent is unenforced — renovate.json automerges a digest re-push after a 3-day soak with no tdarr exclusion) |
