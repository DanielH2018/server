...
def _run(wf, ex, state, window_s=7200):
    return checks.service.n8n_update_streaks(wf, ex, state, N8N_NOW, window_s)
...
def test_n8n_verdict_systemic_pages_before_consecutive():
    ...
    ok, msg = checks.service.n8n_verdict({"A Flow": 2, "B Flow": 2}, 3, 2, 2)
...
def test_n8n_verdict_two_single_transients_not_systemic():
    ...
    ok, _ = checks.service.n8n_verdict({"A Flow": 1, "B Flow": 1}, 3, 2, 2)
    assert ok


def test_n8n_verdict_empty_is_up():
    ok, _ = checks.service.n8n_verdict({}, 3, 2, 2)
    assert ok
...
def test_n8n_missing_stoppedat_falls_back_to_startedat():
    ...
    assert checks.service.n8n_update_streaks(wf, ex, {}, N8N_NOW, 7200) == {
        "Prod Flow": 1
    }
...
def test_queue_warnings_flags_warning_status():
    ...
    offenders = checks.service.queue_warnings(q, "Sonarr")
...
def test_queue_warnings_empty_queue_is_clean():
    assert checks.service.queue_warnings(_queue(), "Radarr") == []
...
def test_queue_warnings_ignores_normal_downloading_item():
    ...
    assert checks.service.queue_warnings(q, "Radarr") == []
...
def test_queue_warnings_flags_error_status():
    ...
    offenders = checks.service.queue_warnings(q, "Radarr")
    assert len(offenders) == 1
...
def test_queue_warnings_import_pending_without_messages_is_ok():
    ...
    assert checks.service.queue_warnings(q, "Sonarr") == []
...
def test_indexers_down_flags_indexer_over_threshold():
    ...
    out = checks.service.indexers_down(status, INX_NAMES, INX_NOW, 30)
...
def test_indexers_down_ignores_sub_threshold_flap():
    ...
    assert checks.service.indexers_down(status, INX_NAMES, INX_NOW, 30) == []


def test_indexers_down_empty_status_is_clean():
    assert checks.service.indexers_down([], INX_NAMES, INX_NOW, 30) == []


def test_indexers_down_null_initial_failure_skipped():
    assert (
        checks.service.indexers_down(_status((1, None)), INX_NAMES, INX_NOW, 30) == []
    )


def test_indexers_down_malformed_initial_failure_skipped():
    assert (
        checks.service.indexers_down(
...
def test_indexers_down_unknown_id_falls_back_to_id_label():
    out = checks.service.indexers_down(
...
def test_indexers_down_skips_ignored_indexer():
    ...
    assert (
        checks.service.indexers_down(status, INX_NAMES, INX_NOW, 30, ignore={"eztv"})
        == []
    )
...
def test_indexers_down_ignore_only_named_indexer():
    ...
    out = checks.service.indexers_down(status, INX_NAMES, INX_NOW, 30, ignore={"eztv"})
...
def test_bazarr_healthy_status_and_health_report_nothing():
    ...
    assert checks.service.bazarr_problems(_HEALTHY_BAZARR, {"data": []}) == []
...
def test_bazarr_empty_peer_version_is_the_stale_key_signal(peer):
    ...
    problems = checks.service.bazarr_problems(status, {"data": []})
...
def test_bazarr_absent_peer_version_is_not_a_problem():
    ...
    """
    assert (
        checks.service.bazarr_problems(
...
def test_bazarr_self_reported_health_issues_are_surfaced():
    ...
    assert checks.service.bazarr_problems(_HEALTHY_BAZARR, health) == [
