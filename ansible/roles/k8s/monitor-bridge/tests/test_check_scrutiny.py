...
def test_scrutiny_fresh_device_is_ok():
    ...
    ok, msg = checks.host_thermal.scrutiny_freshness(
...
def test_scrutiny_archived_device_is_skipped():
    ...
    ok, _ = checks.host_thermal.scrutiny_freshness(
        s, 26, now=datetime(2026, 6, 6, 12, 0, tzinfo=timezone.utc)
    )
    assert ok
...
def test_scrutiny_no_devices_is_down():
    ok, msg = checks.host_thermal.scrutiny_freshness({}, 26)
    assert not ok
...
def test_scrutiny_passing_device_is_healthy():
    ...
    ok, msg = checks.host_thermal.scrutiny_health(s)
    assert ok
    assert "ok" in msg
...
def test_scrutiny_missing_device_status_is_ok():
    ...
    s = _summary(_dev("w1", "nvme0", "2026-06-06T06:00:00Z"))
    ok, _ = checks.host_thermal.scrutiny_health(s)
    assert ok
...
def test_scrutiny_temp_ceiling_flags_only_when_enabled():
    ...
    assert checks.host_thermal.scrutiny_health(s, temp_max=0)[0]  # disabled -> ok
    ok, msg = checks.host_thermal.scrutiny_health(s, temp_max=60)
    assert not ok
...
def test_scrutiny_device_wear_reads_newest_result():
    ...
    assert checks.host_thermal.scrutiny_device_wear(d) == 7


def test_scrutiny_device_wear_missing_field_is_none():
    assert (
        checks.host_thermal.scrutiny_device_wear(
            _details(attrs={"temperature": {"value": 47}})
        )
        is None
    )
    assert checks.host_thermal.scrutiny_device_wear(_details(results=False)) is None
    assert checks.host_thermal.scrutiny_device_wear({}) is None
    assert checks.host_thermal.scrutiny_device_wear(None) is None


def test_scrutiny_device_wear_non_numeric_is_none():
    assert (
        checks.host_thermal.scrutiny_device_wear(_details(attrs=_attr("n/a"))) is None
    )


def test_scrutiny_wear_under_ceiling_is_ok():
    ok, msg = checks.host_thermal.scrutiny_wear_verdict(
        [("nvme0 (SHPP41-500GM)", 7)], 80
    )
    assert ok
...
def test_scrutiny_wear_ceiling_disabled_never_fires():
    ok, msg = checks.host_thermal.scrutiny_wear_verdict([("nvme0", 99)], 0)
    assert ok
...
def test_scrutiny_wear_unreadable_is_inert_not_ok():
    ...
    ok, msg = checks.host_thermal.scrutiny_wear_verdict([("sda (WD40EFRX)", None)], 80)
    assert ok
...
def test_scrutiny_wear_no_devices_is_inert():
    ok, msg = checks.host_thermal.scrutiny_wear_verdict([], 80)
    assert ok
    assert "INERT" in msg
