...
def test_speedtest_fast_completed_run_is_ok():
    ok, msg = checks.host.speedtest_verdict(_st_row(), 100.0, 8.0, now=ST_NOW)
    assert ok
...
def test_speedtest_below_floor_pages_and_names_the_server():
    ok, msg = checks.host.speedtest_verdict(
        _st_row(
...
def test_speedtest_floor_is_exclusive_at_the_boundary():
    ...
    assert checks.host.speedtest_verdict(
        _st_row(download_bits=100_000_000), 100.0, 8.0, now=ST_NOW
    )[0]
    assert not checks.host.speedtest_verdict(
...
def test_speedtest_no_rows_at_all_pages():
    ok, msg = checks.host.speedtest_verdict(None, 100.0, 8.0, now=ST_NOW)
    assert not ok
...
def test_speedtest_disabled_without_url_or_token(monkeypatch):
    monkeypatch.setattr(bridge.config, "SPEEDTEST_URL", "")
    monkeypatch.setattr(bridge.config, "SPEEDTEST_TOKEN", "")
    ok, msg = checks.host.check_speedtest()
    assert ok
...
def test_speedtest_fetch_failure_rides_the_streak_but_a_bad_row_does_not(monkeypatch):
    ...
    monkeypatch.setattr(bridge.config, "SPEEDTEST_URL", "http://speedtest")
    monkeypatch.setattr(bridge.config, "SPEEDTEST_TOKEN", "t")
    monkeypatch.setattr(bridge.config, "SPEEDTEST_CONSECUTIVE", 2)

    def _boom(*a, **k):
        raise RuntimeError("connection refused")

    monkeypatch.setattr(bridge.net, "_get_json", _boom)
    assert checks.host.check_speedtest()[0]  # first failure is held by the streak
    assert not checks.host.check_speedtest()[0]  # second pages
    ...
    assert not checks.host.check_speedtest()[0]  # a bad row pages on the FIRST cycle
