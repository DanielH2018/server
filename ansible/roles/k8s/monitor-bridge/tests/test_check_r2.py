...
def test_r2_month_start_is_utc_first_of_month():
    ...
    assert checks_r2.r2_month_start(now) == datetime(2026, 8, 1, tzinfo=timezone.utc)


def test_r2_classify_splits_the_two_billed_classes():
    a, b, unknown = checks_r2.r2_classify_operations(
...
def test_r2_classify_counts_unknown_actions_as_class_a_and_names_them():
    ...
    a, b, unknown = checks_r2.r2_classify_operations(_ops(PutObject=1, SomeNewOp=42))
    assert (a, b) == (43, 0)
    assert unknown == ["SomeNewOp"]


def test_r2_verdict_up_well_inside_the_free_tier():
    ok, msg = checks_r2.r2_usage_verdict(1_200_000_000, 0, 4_100, 88_000, [])
    assert ok
...
def test_r2_verdict_down_on_storage_breach():
    ok, msg = checks_r2.r2_usage_verdict(8_500_000_000, 0, 0, 0, [])
    assert not ok
...
def test_r2_verdict_down_on_class_b_breach_names_only_that_arm():
    ok, msg = checks_r2.r2_usage_verdict(0, 0, 0, 9_000_000, [])
    assert not ok
...
def test_r2_verdict_breaches_at_exactly_the_threshold():
    ok, _ = checks_r2.r2_usage_verdict(8_000_000_000, 0, 0, 0, [])
    assert not ok
...
def test_r2_verdict_down_on_orphaned_multipart_uploads():
    ...
    ok, msg = checks_r2.r2_usage_verdict(0, 500, 0, 0, [])
    assert not ok
...
def test_r2_verdict_reports_unknown_actions_even_when_up():
    ok, msg = checks_r2.r2_usage_verdict(0, 0, 5, 0, ["SomeNewOp"])
    assert ok
...
def test_r2_verdict_treats_a_zero_limit_as_no_limit():
    ...
    ok, msg = checks_r2.r2_usage_verdict(5_000_000_000, 0, 0, 0, [], storage_max_gb=0)
    assert ok
...
def test_r2_query_parses_storage_and_operations(monkeypatch):
    ...
    assert checks_r2.r2_query_usage(time.time()) == (1000, 3, 2, 8, [])
...
def test_r2_query_treats_an_empty_bucket_as_zero_not_a_fault(monkeypatch):
    ...
    assert checks_r2.r2_query_usage(time.time()) == (0, 0, 0, 0, [])
...
def test_r2_query_raises_on_graphql_errors(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="unauthorized"):
        checks_r2.r2_query_usage(time.time())
...
def test_r2_usage_caches_a_success(monkeypatch):
    ...
    checks_r2.r2_usage(now=1000.0)
    ok, msg = checks_r2.r2_usage(now=1000.0 + bridge_config.R2_PROBE_INTERVAL_S - 1)
    assert ok
    assert len(calls) == 1
...
def test_r2_usage_reprobes_after_a_failure(monkeypatch):
    ...
    assert not checks_r2.r2_usage(now=1000.0)[0]
    assert not checks_r2.r2_usage(now=1001.0)[0]
    assert len(calls) == 2
