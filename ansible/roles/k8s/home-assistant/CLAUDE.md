...
- **Skills** (`.claude/skills/`): `ha-edit-automation` (the authoring workflow — copy-not-template,
  math-in-a-tested-macro, validate→deploy→verify; its step 5 also carries live state via the
  API and the recorder + alias-slug traps), `z2m-device-setting`
...
- **Derived state model** (`state/STATE.md` + `state/derived_state.yml`, generated by
  `scripts/home_assistant/ha_state_model.py generate`): the machine-derived map of cells/actuators and who
...
`scripts/home_assistant/validate_ha_config.py` resolves every entity reference against config refs ∪
