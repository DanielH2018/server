...
- `copy`s the tracked static `files/.gitconfig` and `files/.bashrc` to `$HOME`
  (`dest` uses `ansible_facts.env.HOME`, `mode 0644`, `backup: true` → an existing file is
  preserved as a timestamped `.bak` when content changes).
