...
`test_lock_names_agree_with_deploy_sh_is_clean` pins the lock names to the literals in
`deploy.sh`, because the daemon runs outside the venv and cannot import them.
