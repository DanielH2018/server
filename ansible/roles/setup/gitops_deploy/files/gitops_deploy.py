...
def _failing_task(text: str) -> tuple[str, str] | None:
...
def _failure_detail(stdout: str, limit: int) -> str:
    ...
    """
    found = _failing_task(stdout)
    if found is None:
        return _tail(stdout, limit)
    task, rest = found
    rest, _, _ = rest.partition("\nTASKS RECAP")
    task = _head(task, limit)
    rest = _tail(rest, limit - len(task))
...
def _alert_excerpt(exc: BaseException, limit: int = ALERT_EXCERPT_CHARS) -> str:
    ...
    found = _failing_task(body)
    if found is None:
        return f"{head}\n{_tail(body, budget)}"
    return f"{head}\n{_head(found[0], budget)}"
...
def run(
    ...
    if check and r.returncode != 0:
        detail = "\n".join(
            part
            for part in (
                _failure_detail(r.stdout, RUN_ERROR_STDOUT_CHARS),
