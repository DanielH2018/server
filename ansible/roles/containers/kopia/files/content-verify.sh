...
write_state() { # ok msg
  # jq, not printf: a stray backslash/control char in the kopia-derived msg would make a hand-built
  # string invalid JSON -> monitor-bridge reads "state unparseable" (false DOWN). jq escapes it.
  jq -nc --argjson ts "$(date +%s)" --argjson ok "$1" --arg msg "$2" \
    '{ts: $ts, ok: $ok, msg: $msg}' > "$STATE"
  logger -t kopia-content-verify "$1: $2"
}
