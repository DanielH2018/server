...
- **The karakeep app image is digest-pinned.** `:release` is a mutable floating tag (no semver for
  Renovate to order) and Compose never re-pulls a present tag, so a plain redeploy leaves it stale
  with no signal — hence the `repo:release@sha256` pin. Because the **tag is kept**, Renovate's docker
  manager still matches it and opens tracked digest-bump PRs (contrast tdarr/janitorr, which are bare
  `repo@sha256` with no tag = `DIGEST_PINNED_EXEMPT`, zero Renovate tracking, fully manual).
  **To update by hand:** `docker pull ghcr.io/karakeep-app/karakeep:release`, take the new digest,
  update the `image:` line, redeploy — checking the meili VERSION POLICY still holds for the new version.
