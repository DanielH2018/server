...
@dataclass
class Row:
    """One row of the recurring-open register table.

    Attributes:
        item: the row's prose — the finding, as written in the register.
        first_seen: the date the register first recorded the row.
        runs_carried: how many review runs have carried the row forward.
    """
...
@dataclass
class RowResult:
    """The audit's verdict for one register row.

    Attributes:
        reference: the evidence the verdict is based on, or None when the row's prose
...
def classify_row(row: Row, repo: Path, files: list[Path]) -> RowResult:
    """Classify one register row as likely-closed, open, or unresolved.

    Tries each evidence tier in order — A (a `check_X` identifier resolving to a
    `test_X` fixture), B (bolded/proper-noun keywords co-occurring in a repo path), then
    C (a bare path/path:line/identifier citation, which anchors the current gap rather
    than proving a fix) — and returns on the first tier that yields a verdict.
    """
    item = row.item
...
def format_report(results: list[RowResult], repo: Path) -> str:
    """Render `results` as the human-readable audit report, one section per status."""
...
def main(argv: list[str] | None = None) -> int:
    """Audit the register and print the report. Always exits 0 — this is a report, not a gate."""
