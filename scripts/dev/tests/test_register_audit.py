...
def test_likely_closed_via_fixture_heuristic(tmp_path):
    """The check_b2_storage case:

    no path is cited at all, but this repo's convention gives a `check_X` function a `test_X.py`
    fixture.
    """
    register = make_register(
        tmp_path,
...
def test_likely_closed_via_keyword_search(tmp_path):
    """The Longhorn Grafana dashboard case:

    the citation (`check.py:2738`) is the pre-existing check arm, not the missing dashboard; closure
    evidence is a *different* path matching the row's own vocabulary.
    """
    register = make_register(
        tmp_path,
