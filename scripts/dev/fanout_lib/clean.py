...
def clean_one(
    ...
    13. This unlocks the tree FIRST, so classify sees it unlocked and decides on merged/dirty
    state instead, then re-locks with its original reason when the verdict is still KEEP —
    so a batch that is still running or still unmerged stays protected from an unrelated
    prune the same way launch intended. A tree that was never locked to begin with is left
    unlocked either way.
    ...
    Args:
        ...
        unlocker: unlocks `tree` before classify sees it.
        locker: re-locks `tree` with its original reason when it stays kept.
    ...
    return ("removed", "") if ok else ("kept", err)
...
def remote_clean_command(b: Batch) -> str:
    ...
    (`<b.worktree>/scripts/dev/fanout_place.py`), not the primary checkout's, because
    daniel-server's primary checkout is stale by design and the fresh worktree is the only
    checkout there guaranteed to carry this code (Ruling E). It runs from the primary
    checkout's cwd (`cd {REPO}`) so a relative path elsewhere in the command still resolves
    there.
    """
    return (
        f"systemctl --user reset-failed {b.unit} 2>/dev/null; "
        f"cd {REPO} && uv run python {b.worktree}/scripts/dev/fanout_place.py "
        f"clean-one {b.worktree} {b.branch}"
    )
