"""The page's stylesheet, its status vocabulary, and the escape every view calls.

Catppuccin Mocha, matching the terminal these pages are generated from. The CSS is
inlined into the page because the output has to open over ``file://``, and the
standalone SVG carries a copy of it for the same reason.

The other view modules read from here; this one reads from none of them.
"""
