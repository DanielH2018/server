...
# --- readonly-rbac: is the read-only ServiceAccount still read-only? ---------------------------
...
# --- vip-placement: does every ETP=Local VIP have a Ready endpoint on an announcing node? ------
...
# endpoint installs a filter-table KUBE-EXTERNAL-SERVICES DROP for it. So when the L2
# announcer and the backing pod sit on different nodes, every forwarded LAN packet dies at the
# announcer — silently, with the Service Ready and the pod 1/1.
#
