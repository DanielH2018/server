...
# resolved arguments:
#
#   deploy_locked.sh <at-sha> <detach 0|1> <tags-csv> -- <ansible-playbook args...>
#
...
# rewrites the tree every other deploy renders from. This script holds it only long enough to
...
# services run at once. A run that names no tag takes server-deploy-all.lock exclusively and
# every scoped run takes it shared, so a full run and a scoped run still exclude each other.
#
# The tree-lock wait is deliberately longer than gitops-deploy's own 180s: an interactive
# deploy should queue behind the unattended pipeline rather than give up. If this run holds the
# lock for more than 180s, the next gitops firing fails its unit and raises a Discord alert.
# That alert is accurate and the timer retries 10 minutes later, so it is left as a true signal.
# The snapshot makes that hold short enough that it should be rare.
#
...
# record's `tree_dirty` structurally false and what makes it safe to let the tree move while
# the playbook runs.
#
# Exit codes. 0 is a finished deploy; 79, 78, 77, 76 and 75 each mean NOTHING was deployed and
# each is a resume point; 20 means the playbook RAN and a task failed, so whatever applied
# before it is live. ansible-playbook's own status collides with the wrapper's 2/3/4 and is
# collapsed onto 20, see PLAYBOOK_FAILED below. The full table is `exit_codes.py`.

set -u

# deploy_run.py cleared O_NONBLOCK on the descriptors this process inherits (#834), so
# ansible-playbook below gets blocking stdio without a fixup here.
...
#
# Fails open, like the fact-cache preflight in deploy_run.py: a snapshot this run cannot reap costs
...
# The checkout deploy_run.py resolved and changed into: the caller's, not this file's.
repo_root=$(git rev-parse --show-toplevel)
cd "$repo_root" || exit 1

if [[ $# -lt 4 || "$4" != "--" ]]; then
    echo "deploy_locked.sh: run ./scripts/deploy.sh, not this script -- nothing was deployed." >&2
    exit 64
fi
at_sha="$1"
detach="$2"
...
if [[ -n "$split_tags_csv" ]]; then
    IFS=, read -r -a split_tags <<<"$split_tags_csv"
fi
