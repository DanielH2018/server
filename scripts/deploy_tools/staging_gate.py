...
def classify(rc: int) -> int:
    ...
    instead of asserting arithmetic of its own. A busy staging lock is NO_VERDICT like any other
    reason the gate could not be asked: the remote reports it as PREP_FAILED.
    """
    if rc == PASS:
        return PASS
...
def main() -> int:
    ...
    Exits PASS (0), REJECTED (1) or NO_VERDICT (2).
    """
